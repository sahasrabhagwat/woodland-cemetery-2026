...
window.onload = () => {
  ...
  new Promise(async (resolve) => {
    ...
    form.onsubmit = (evt) => {
      // Hiding the currently highlighted graves unless in "show all" mode
  ...
  // Adding event listener for the "Show all graves" toggle
  const showAllToggle = document.getElementById("show-all-graves-toggle");
  if (showAllToggle) {
    showAllToggle.addEventListener("change", () => {
      showAllGraves = showAllToggle.checked;
      ...
      // Update row polygons
      const zzPolygons = document.querySelectorAll('[class^="ZZ "]');
      ...
      const dPolygons = document.querySelectorAll('[class^="D "]');
      for (let polygon of dPolygons) {
        if (showAllGraves) {
          polygon.style.opacity = 1;
        } else {
          polygon.style.opacity = 0;
      ...
      const mhillside = document.getElementsByClassName("MHillside")[0];
      if (mhillside) {
        if (showAllGraves) {
          mhillside.style.opacity = 1;
        } else {
          mhillside.style.opacity = 0;
        }
      }
    });
  }
  ...
  for (let grave of graves) {
    ...
    grave.onmouseleave = () => {
      label.style.display = "none";
      // Only hide grave if "show all" mode is not enabled
      if (showAllGraves) {
        grave.style.opacity = 1;
      } else {
        grave.style.opacity = 0;
  ...
  if (hillsideSection) {
    ...
    hillsideSection.onmouseleave = () => {
      label.style.display = "none";
      // WIP: keep graves visible (not fully integrated yet)
      if (showAllGraves) {
        hillsideSection.style.opacity = 1;
      } else {
